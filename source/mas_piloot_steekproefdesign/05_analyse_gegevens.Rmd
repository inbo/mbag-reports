...
\textcolor{red}{waarom veldleeuwerik ... De resulaten van andere soorten zullen in een ander rapport worden beschreven.}
...
include_graphics(file.path("media", "distance", "per_stratum",
                           "detectiekans_per_stratum_veldleeuwerik.jpg"))
```
...
Er zijn ook andere manieren om densiteiten in te berekenen, zoals o.b.v. het maximum aantal broedparen over de telperiodes heen.
Dit kan dan omgerekend worden naar densiteiten door te delen door de bezochte oppervlakte.
De maxima komen goed overeeen met de resultaten van distance sampling (Fig. \@ref(fig:densiteit-methodiek)).
...
\textcolor{red}{eigenlijk niet het geval omdat wij door 4 delen!} Wij doen dat wel via distance sampling, maar in het geval van de Veldleeuwerik zien we hier geen groot verschil tussen het gebruik van maxima of distance sampling.
Het voordeel is wel dat we via distance samping alle data gebruiken.
\textcolor{red}{zie mail en leg figuur uit}

```{r densiteit-methodiek, out.width = "90%", fig.cap = "Vergelijking van twee methodes om densiteiten te schatten. Zie tekst voor meer uitleg."}
include_graphics(file.path("media", "distance", "per_stratum",
                           "densiteit_per_stratum_max_veldleeuwerik.jpg"))
```
